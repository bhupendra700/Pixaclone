...
const Main = ({ data, error, ref, isFetching }) => {
      ...
      <div className="upper-wrapper">
        ...
        <div className={size < 1025 ? "filter2-mob" : "filter2-web"}>
          <details>
            ...
            <div>
              <span className={(searchParam.get("order") !== "latest" && searchParam.get("order") !== "popular") ? 'cat-3 green' : 'cat-3'} onClick={() => orderFilter("ec")}>
                Editor's Choice
              </span>
              <span className={searchParam.get("order") === "latest" ? 'cat-1 green' : 'cat-1'} onClick={() => {
                orderFilter("latest")
              }}>
                Latest
              </span>
  ...
  if (data && data.pages[0].hits.length === 0) return <div className='main-div'>
    <div className="upper">
      <div className="upper-wrapper">
        <div className={size < 1025 ? "filter1-mob" : "filter1-web"}>
          {size < 1025 && <button onClick={() => { setFilter(!filter) }}><i className="ri-equalizer-2-fill"></i> Filters {filterCount !== 0 && <span>({filterCount})</span>}</button>}
  ...
  return <div className='main-div'>
    ...
    <div className="middle">
      <h2>{data && data.pages[0].total.toLocaleString("en-IN")} Free {cat.substring(0, 1).toUpperCase() + cat.substring(1)}</h2>
    </div>
    <div className="lower">
